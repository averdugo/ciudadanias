		...
		$output = sprintf(
			'<div%3$s class="et_pb_testimonial%4$s%5$s%9$s%10$s%12$s clearfix"%11$s>
				...
				<div class="et_pb_testimonial_description">
					...
					<strong class="et_pb_testimonial_meta" style="font-size: 11px;">%6$s%7$s</strong>
		...
		if ( have_posts() ) {
			...
			<?php if ( 'off' === $fullwidth || ! in_array( $post_format, array( 'link', 'audio', 'quote' ) ) ) { ?>
				...
				<?php
					...
					if ( 'on' !== $show_content ) {
						$more = 'on' == $show_more ? sprintf( ' <a href="%1$s" class="more-link" >%2$s</a>' , esc_url( get_permalink() ), esc_html__( 'LEER MAS', 'et_builder' ) )  : '';
						echo $more;
					}
					?>
